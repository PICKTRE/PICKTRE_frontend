  ...
  const run = async () => {
    ...
    const loop = async () => {
      ...
      if (currentTime - lastFrameTime >= frameInterval) {
        ...
        //     try {
        //       const createResponse = await cameraReward(2, "medical", 500);
        ...
        if (result.dataSync()[3] >= 0.8) {
          // alert("성공");
          try {
            const createResponse = await cameraReward(memberId, "medical", 500);
            console.log(createResponse);
            alert("확인되었습니다.");
            navigate("/home");
            return () => {
...
export default TrashcameraMedical;
